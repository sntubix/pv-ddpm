...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x, t, condition_tensors=None, clip_denoised=True, repeat_noise=False):
        ...
        Args:
            ...
            t: Current timestep
            condition_tensors: Conditioning input (e.g., masks)
        """
    ...
    @torch.no_grad()
    def p_sample_loop_from_image(self, init_image_path, condition_tensors=None):
        """
        Sample from the model starting from a specific image
        Args:
            init_image_path: Path to initial image
            condition_tensors: Conditioning tensors (masks) [B, C, H, W]
        """
        device = self.betas.device
        ...
        init_tensor = transform(init_image).unsqueeze(0)
        
        # Repeat the initial image to match batch size of condition_tensors
        b = condition_tensors.shape[0]
        init_tensor = init_tensor.repeat(b, 1, 1, 1).to(device)
        
        # Start sampling loop
        img = init_tensor
        for i in tqdm(reversed(range(self.num_timesteps)), desc='sampling loop time step'):
            t = torch.full((b,), i, device=device, dtype=torch.long)
            img = self.p_sample(img, t, condition_tensors=condition_tensors)
    ...
    @torch.no_grad()
    def sample_from_image(self, init_image_path, condition_tensors=None):
        """
        Wrapper method for sampling from a specific image
        """
        return self.p_sample_loop_from_image(init_image_path, condition_tensors=condition_tensors)
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        Args:
            ...
            t: Timestep
            noise: Random noise (generated if None)
        """
    ...
    def p_losses(self, x_start, t, condition_tensors = None, noise = None):
        ...
        if self.loss_type == 'l1':
            ...
            x_start_hat = self.predict_start_from_noise(x_noisy, t=t, noise=x_recon)
            perceptual_loss = PerceptualLoss()(x_start_hat, x_start)
            loss = F.mse_loss(x_recon, noise) + self.perceptual_weight * perceptual_loss
        else:
...
class Trainer(object):
    ...
    def save(self, milestone):
        ...
        torch.save(self.model, str(self.results_folder / f'full_model-{milestone}.pth'))
    ...
    #     """
    #     Denormalize the image tensor from [-1, 1] to [0, 255] and convert it to [B, 3, H, W].
    #     """
    #     # Denormalize from [-1, 1] to [0, 255]
    #     image_tensor = (image_tensor + 1) * 0.5 

    #     # Duplicate the single channel into 3 channels (RGB-like)
    #     image_tensor = image_tensor.repeat(1, 3, 1, 1) 

    #     # Add an alpha channel with all values set to 255
    #     # alpha_channel = torch.full((image_tensor.size(0), 1, image_tensor.size(2), image_tensor.size(3)),255, dtype=torch.uint8, device=image_tensor.device)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for i in range(0, len(self.val_dataset), self.batch_size):
                batch = [self.val_dataset[j] for j in range(i, min(i + self.batch_size, len(self.val_dataset)))]
                input_batch = torch.stack([sample['input'] for sample in batch]).cuda()
                target_batch = torch.stack([sample['target'] for sample in batch]).cuda()
                t = torch.randint(0, self.model.num_timesteps, (input_batch.size(0),), device=input_batch.device).long()
                loss = self.model.p_losses(target_batch, t, condition_tensors=input_batch)
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                # all_images = self.denormalize(all_images)
                utils.save_image(all_images, str(self.results_folder / f'sample-{milestone}.png'), nrow = 2)
                self.save(milestone)
                
                training_info['samples'].append((self.step, sample_path))
                training_info['models'].append((self.step, model_path))
            
            # Validation step
            if self.val_dataset and self.step % self.save_and_sample_every == 0:
                val_loss = self.evaluate()
                print(f"Validation Loss at step {self.step}: {val_loss}")
                training_info['val_losses'].append((self.step, val_loss))
